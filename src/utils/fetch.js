...
function FetchError(message, name) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const req = http.request(request.url, {
      ...
      res.on('end', () => {
        ...
        options.url = ('url' in request) ? request.url : options.headers.get('X-Request-URL')
    ...
    req.on('abort', (err) => {
      reject(new FetchError('Aborted', 'AbortError'))
    });
    ...
    // if (request.signal) {
    //   request.signal.addEventListener('abort', () => req.abort)
    //
    //   xhr.onreadystatechange = function() {
    //     // DONE (success or failure)
    //     if (xhr.readyState === 4) {
    //       request.signal.removeEventListener('abort', () => req.abort)
    //     }
    //   }
    // }
